...
<template>
  <header className="rounded-lg p-3 bg-white flex text-gray-200 justify-between items-center">
    <img v-if="matches" src="@/assets/icons/logo-devlinks-small.svg" alt="devlink logo" width="25" height="25" />

    <img v-else src="@/assets/icons/logo-devlinks-large.svg" alt="devlink logo" width="125" height="125" />
    <div className="flex gap-2">
      <button :class="[isActive === 'links' ? 'active' : 'hover:text-purple-300']"
        class="px-4 py-2 flex gap-1 text-gray rounded-lg text-sm" @click="toggleActive('links')">
        <LinkIcon />
        <span v-if="!matches">Links</span>
      </button>
      <button :class="[isActive === 'profile' ? 'active' : 'hover:text-purple-300']"
        class="px-4 flex gap-1 py-2 text-gray rounded-lg text-sm" @click="toggleActive('profile')">
        <ProfileIcon />
